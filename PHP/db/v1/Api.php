	...
	if(isset($_GET['apicall'])){
		...
		switch($_GET['apicall']){
			...
			case 'createreport':
				...
				isTheseParametersAvailable(array('owner','pet_name','last_seen','contact','description','photo','location','tag'));
				
				//creating a new dboperation object
				$db = new DbOperation();
				//creating a new record in the database
				$result = $db->createReport(
					$_POST['owner'],
					$_POST['pet_name'],
					$_POST['last_seen'],
					$_POST['contact'],
				...
				if($result){
					...
					$response['message'] = 'Report addedd successfully';

					//and we are getting all the heroes from the database in the response
					$response['reports'] = $db->getReports();
				}else{
...
//				json_encode(($response['reports'] = $db->getReports()),JSON_UNESCAPED_SLASHES);
				$response = $db->getReports();
